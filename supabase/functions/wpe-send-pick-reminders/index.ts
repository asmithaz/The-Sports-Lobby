// CFB Weekly Pick 'Em — pick reminder emails
//
// For every active/pending league, finds the current week's earliest kickoff
// and emails any member who opted into a reminder (wpe_reminder_prefs,
...
// interval), ever — wpe_reminder_log's unique constraint is the
// dedup/claim mechanism (see schema.sql SECTION 10), so polling every
// 15 minutes (more often than any reminder needs to fire) is harmless.
//
// Uses wpe_get_slate_unchecked (schema.sql SECTION 9) instead of the
// regular wpe_get_slate RPC — this is a service-role job with no real
// user session, and wpe_get_slate's own membership check would reject
// every call (auth.uid() is NULL under a service-role key).
//
// Triggered by pg_cron + pg_net every 15 min (schema.sql SECTION 11),
// same pattern as wpe-sync-games/bpe-sync-games. Emails go straight
// through Resend, not the shared send-email Edge Function (which
// requires a logged-in user's JWT this cron doesn't have) — same
// reasoning bpe-sync-games's notifyMatchupChanges already applies.
//
// Call with ?dry_run=true to see what WOULD be sent/claimed without
// actually sending or writing to wpe_reminder_log.
...
const LOCK_BUFFER_MS = 5 * 60 * 1000; // same 5-minutes-before-kickoff rule as wpe_is_game_locked
...
async function sendForLeague(supabase: any, league: { id: string; name: string; current_season: number }, week: number, dryRun: boolean, results: any[]) {
  const { data: weekGames } = await supabase.rpc("wpe_get_slate_unchecked", {
  ...
  const { data: wpeLeagueRow } = await supabase
    ...
    .maybeSingle();
  const needsConfidence = wpeLeagueRow?.scoring_mode === "confidence";
    ...
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: `
          ...
          <p><a href="https://thesportslobby.com/cfb/weekly-pick-em/picks/?league_id=${league.id}" style="display:inline-block;padding:10px 20px;background:#4ade80;color:#0f1117;font-weight:600;border-radius:6px;text-decoration:none;">Make Your Picks</a></p>
        `,
      }),
    });
...
async function sendPickReminders(supabase: any, dryRun: boolean) {
  ...
  // Same status set wpe_recalculate_all_scores treats as live — a new
